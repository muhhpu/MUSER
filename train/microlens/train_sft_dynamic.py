# -*- coding: utf-8 -*-
# LLaVA 训练脚本（answer+reason 双重 loss，已修正若干细节 + 动态loss权重）
...
SAVE_DIR = f"/home/team//MLLM-MSR-main/MLLM-MSR/save/LLaVA/llava-v1.6-mistral-7b-hf-lora-recurrent-user-longshort-finetunereason-{location}-dynamic-e{EPOCH}-r{LORA_R}"
...
class LlavaModelPLModule(L.LightningModule):
    ...
    ...  # （这里保持 training_step 和 validation_step 不变，和你上传的版本一致）
...
class DynamicLossWeightCallback(Callback):
    def __init__(self, monitor="val_answer_acc", patience=2, min_answer_weight=0.5, step=0.1):
        super().__init__()
        self.monitor = monitor
        self.patience = patience
        self.min_answer_weight = min_answer_weight
        self.step = step
    ...
    def on_validation_end(self, trainer, pl_module):
        ...
        if self.bad_epochs >= self.patience:
            if pl_module.answer_weight > self.min_answer_weight:
                new_answer = max(self.min_answer_weight, pl_module.answer_weight - self.step)
                new_reason = 1.0 - new_answer
                pl_module.answer_weight = new_answer
                pl_module.reason_weight = new_reason
                print(f"[DynamicLossWeight] 调整权重: answer={new_answer:.2f}, reason={new_reason:.2f}")
            self.bad_epochs = 0
...
model_module = LlavaModelPLModule(config, processor, model,
                                  answer_weight=1.0, reason_weight=0.0)
...
dynamic_weight_callback = DynamicLossWeightCallback(
    monitor="val_answer_acc", patience=2, min_answer_weight=0.5, step=0.1
)
...
trainer = L.Trainer(
    ...
    callbacks=[SaveToDiskCallback(), early_stop_callback, checkpoint_callback, dynamic_weight_callback]
)
