...
SAVE_DIR = f"/home/team//MLLM-MSR-main/MLLM-MSR/save/LLaVA/llava-v1.6-mistral-7b-hf-lora-recurrent-user-longshort-finetunereason-{location}-linear1-e{EPOCH}-r{LORA_R}-new-llm全layer-numeric-notbad"
...
class LlavaModelPLModule(L.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        for b_idx, lab in enumerate(labels_cpu):
            ...
            # ================================================================= #
            #       ★ 一致性惩戒：加入正确度比值门控机制（你要的 3+4）       #
            # ================================================================= #
            lambda1 = float(self.config.get("lambda1", 0.01))
            lambda2 = float(self.config.get("lambda2", 0.01))
            tau_high = float(self.config.get("tau_high", 2))  # reason 明显好于 label
            tau_low = float(self.config.get("tau_low", 0.08))  # reason 明显差于 label
            ...
            with torch.no_grad():
                ...
                # 正确度比值：>1 表示 reason 更好；<1 表示 label 更好
                ratio = S_r / (S_y + eps)

                # 门控：ratio 高 → reason 远好于 label；ratio 低 → reason 远逊于 label
                mask_ry = (ratio > tau_high).float()  # reason 强，label 弱
                mask_yr = (ratio < tau_low).float()  # reason 弱，label 强
            ...
            # ===== 动态权重 ===== #
            alpha_dyn = self.alpha + C_ry  # label loss 的权重
            beta_dyn = self.beta + C_yr  # reason loss 的权重
            ...
            self.log("mask_ry", mask_ry.detach(), on_step=True)
            self.log("mask_yr", mask_yr.detach(), on_step=True)
