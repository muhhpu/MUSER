...
train_pair_file_path = "/home/team//MLLM-MSR-main/MLLM-MSR/data/ninerec/Downstream_datasets/Bili_Movie/preprocessing/processed_bili/train_pairs.csv"
df_train = pd.read_csv(train_pair_file_path)
df_train['item'] = df_train['item_id'].astype(str)
df_train['user'] = df_train['user_id'].astype(str)

val_pair_file_path = "/home/team//MLLM-MSR-main/MLLM-MSR/data/ninerec/Downstream_datasets/Bili_Movie/preprocessing/processed_bili/val_pairs.csv"
df_val = pd.read_csv(val_pair_file_path)
df_val['item'] = df_val['item_id'].astype(str)
df_val['user'] = df_val['user_id'].astype(str)


# user_pref_file_path = "/home/team//MLLM-MSR-main/MLLM-MSR/Inference/ninerec/user_preference_noLS_ninerec.csv"
user_pref_file_path = "/home/team//MLLM-MSR-main/MLLM-MSR/Inference/ninerec/user_preference_LS_ninerec_numeric.csv"
...
item_title_file_path = "/home/team//MLLM-MSR-main/MLLM-MSR/data/ninerec/Downstream_datasets/Bili_Movie/Bili_Movie_item.csv"
item_title_df = pd.read_csv(item_title_file_path, header=None, names=["item", "title","title_en"])
item_title_df['item'] = item_title_df['item'].astype(str)


folder_path = "/home/team//MLLM-MSR-main/MLLM-MSR/data/ninerec/Downstream_datasets/Bili_Movie/Bili_Movie_cover"
...
reasons_file = "/home/team//MLLM-MSR-main/MLLM-MSR/deepseek/ninerec/ninerec_train_pairs_with_reasons_numeric_merged.csv"
...
df_val = pd.merge(df_val, reasons_df, on=['user', 'item', 'label'], how="left")
...
df_train['prompt'] = df_train.apply(lambda x: prompt_text.format(x['preference'], x['title_en']), axis=1)
...
df_val['prompt'] = df_val.apply(lambda x: prompt_text.format(x['preference'], x['title_en']), axis=1)
df_val['ground_truth'] = df_val.apply(lambda x: 'Yes' if x['label'] == 1 else 'No', axis=1)
df_val['truth_reason'] = df_val['answer']
...
# dataset.save_to_disk("ninerec-training-recurrent-noLS-yesno-imagepath")
dataset.save_to_disk("ninerec-training-recurrent-longshortpreference-reason-numeric")
