...
# ⚠️ 更新保存路径以反映新模型
SAVE_DIR = f"/home/team//MLLM-MSR-main/MLLM-MSR/save/Qwen2.5-VL/movie-qwen2.5-vl-3b-lora-recurrent-user-longshort-finetunereason-{location}-linear1-e{EPOCH}-r{LORA_R}-new-llm全layer-numeric-双惩loss"
...
# ------------------ Dataset -------------------
dataset_path = "/home/team//MLLM-MSR-main/MLLM-MSR/movielens-training-recurrent-longshortpreference-reason-numeric-imagepath"
...
class QwenVLModelPLModule(L.LightningModule):
    ...
    def on_train_batch_start(self, batch, batch_idx):
        ...
        self.beta = torch.nn.Parameter(torch.tensor(0.1))  # 初始 0.5
    ...
    def training_step(self, batch, batch_idx):
        ...
        # ====== ★ 一致性惩戒部分（新增） ======
        # 假设 answer_loss 可以代表 label 任务正确性（越低越正确）
        # reason_loss 代表 reason 任务正确性（越低越正确）
        # 我们定义归一化正确度指标
        with torch.no_grad():
            S_y = torch.exp(-answer_loss)  # label 正确度（越大越正确）
            S_r = torch.exp(-reason_loss)  # reason 正确度

        # 定义惩戒强度系数（你可以调节）
        lambda1 = 0.01  # reason好但label错 → 增强label监督
        lambda2 = 0.01  # label好但reason错 → 增强reason监督

        # reason 正确但 label 错误时 (S_r 高, S_y 低)
        C_ry = lambda1 * S_r * (1 - S_y)
        # reason 错误但 label 正确时 (S_r 低, S_y 高)
        C_yr = lambda2 * (1 - S_r) * S_y
        C_ry = C_yr = 0
        # 动态调整权重
        alpha_dyn = self.alpha + C_ry
        beta_dyn = self.beta + C_yr

        # ====== 最终loss ======
        loss = alpha_dyn * answer_loss + beta_dyn * reason_loss
        # ====== 查看梯度 ======
        params = [p for p in self.model.parameters() if p.requires_grad]
        grads_a = torch.autograd.grad(answer_loss, params, retain_graph=True, allow_unused=True)
        grads_r = torch.autograd.grad(reason_loss, params, retain_graph=True, allow_unused=True)

        # 过滤 None，再求范数
        grads_a = [g for g in grads_a if g is not None]
        grads_r = [g for g in grads_r if g is not None]

        grad_a = torch.norm(torch.stack([torch.norm(g) for g in grads_a]))
        grad_r = torch.norm(torch.stack([torch.norm(g) for g in grads_r]))

        print(f"grad ratio: {grad_r / (grad_a + 1e-8):.3f}")
        # ====== Logging ======
        self.log("train_answer_loss", answer_loss, on_step=True, prog_bar=True)
        self.log("train_reason_loss", reason_loss, on_step=True, prog_bar=True)
        self.log("train_loss_total", loss, on_step=True, prog_bar=True)
