...
os.environ["CUDA_VISIBLE_DEVICES"] = "1"  # <--- 关键修改：固定只使用设备 3
DEVICE = torch.device(
...
# PEFT_MODEL_ID = f"/home/team//MLLM-MSR-main/MLLM-MSR/save/LFM2-VL/ninerec-lfm2-vl-450m-lora-yesno_lfm2-e2-r8"
# dataset = load_from_disk(f"/home/team//MLLM-MSR-main/MLLM-MSR/ninerec-test-recurrent_noLS-yesno-imagepath")

PEFT_MODEL_ID = f"/home/team//MLLM-MSR-main/MLLM-MSR/save/LFM2-VL/ninerec-lfm2-vl-450m-lora-recurrent-user-longshort-finetunereason-kua-linear1-e2-r8-numeric"
dataset = load_from_disk(f"/home/team//MLLM-MSR-main/MLLM-MSR/ninerec-test-recurrent-longshortpreference-reason-new-numeric-imagepath")
...
def gpu_computation(examples):
    ...
    for prompt_text in examples['prompt']:
        # --- FIX START ---
        # The dataset prompt already has an <image> tag.
        # We replace it with an empty string to avoid having two <image> tags in the final text.
        clean_prompt = prompt_text.replace("<image>", "")

        # Now construct the text with a single <image> tag at the start
        text = f"USER: <image>\n{clean_prompt}\nASSISTANT:"
        # --- FIX END ---
...
if __name__ == '__main__':
    ...
    recommendation_list_length = 12  # 假设每个用户有21个item
