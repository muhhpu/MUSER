...
SAVE_DIR = f"/home/team//MLLM-MSR-main/MLLM-MSR/save/LLaVA/llava-v1.6-mistral-7b-hf-lora-recurrent-user-longshort-finetunereason-{location}-linear1-e{EPOCH}-r{LORA_R}-new-llm全layer-numeric-双惩loss"
...
train_dataset = LlavaDataset2(
    "/home/team//MLLM-MSR-main/MLLM-MSR/MicroLens-50k-training-recurrent-longshortpreference-reason-new-numeric",
...
class LlavaModelPLModule(L.LightningModule):
    def __init__(self, config, processor, model, answer_weight=1.0, reason_weight=0.0):
        ...
        self.alpha = torch.nn.Parameter(torch.tensor(1.0))   # label loss 权重基准
        self.beta = torch.nn.Parameter(torch.tensor(0.5))    # reason loss 权重基准

        # ================================
        # ★ 可学习阈值（log-space）
        # ================================
        import math
        self.log_tau_high = torch.nn.Parameter(torch.tensor(math.log(1.2)))  # 初始 tau_high = 1.2
        self.log_tau_low  = torch.nn.Parameter(torch.tensor(math.log(0.8)))  # 初始 tau_low  = 0.8
    ...
    def training_step(self, batch, batch_idx):
                ...
                if reason_pos != -1 and reason_pos > ans_pos:
                    ...
                    for i in range(len(lab_clean) - 1, -1, -1):
                        if lab_clean[i] != pad_id:
                            ...
                            break
                    ans_end = last_nonpad
                s, e = max(ans_start - 1, 0), min(ans_end - 1, token_losses.size(1))
            ...
            if reason_pos != -1:
                ...
                for i in range(len(lab_clean) - 1, -1, -1):
                    if lab_clean[i] != pad_id:
                        ...
                        break
                r_end = last_nonpad
                s, e = max(r_start - 1, 0), min(r_end - 1, token_losses.size(1))
        ...
        answer_loss = ((token_losses * batch_answer_mask).sum(dim=1) / answer_counts).mean()
        reason_loss = ((token_losses * batch_reason_mask).sum(dim=1) / reason_counts).mean()

        # =================================================================
        # ★ 一致性惩戒 + 可学习阈值 tau_high / tau_low
        # =================================================================
        lambda1 = 0.3  # reason 好但 label 差 → 惩罚 label
        lambda2 = 0.1  # label 好但 reason 差 → 惩罚 reason

        # ---- 正确度（范围 0~1）----
        with torch.no_grad():
            S_y = torch.exp(-answer_loss)  # label correctness
            S_r = torch.exp(-reason_loss)  # reason correctness
            ...
            # 可学习阈值（用 exp 转回正数）
            # ------------------------------
            tau_high = torch.exp(self.log_tau_high)
            tau_low  = torch.exp(self.log_tau_low)

            # ------------------------------
            # Hard gating（detach 避免梯度穿透 step）
            # ------------------------------
            mask_ry = (ratio > tau_high).float().detach()  # reason >> label
            mask_yr = (ratio < tau_low).float().detach()   # label >> reason

            # 原始惩戒强度
            base_C_ry = lambda1 * S_r * (1 - S_y)
            base_C_yr = lambda2 * (1 - S_r) * S_y

            # 门控惩罚
            C_ry = base_C_ry * mask_ry
            C_yr = base_C_yr * mask_yr

        # ====== 动态 loss 权重 ======
        alpha_dyn = self.alpha + C_ry   # label 任务的权重
        beta_dyn  = self.beta + C_yr    # reason 任务的权重

        # ====== 最终 Loss ======
        loss = alpha_dyn * answer_loss + beta_dyn * reason_loss

        # ===== Logging =====
        self.log("train_answer_loss", answer_loss, on_step=True)
        self.log("train_reason_loss", reason_loss, on_step=True)
        self.log("train_loss_total", loss, on_step=True)
        self.log("alpha_dyn", alpha_dyn.detach(), on_step=True)
        self.log("beta_dyn", beta_dyn.detach(), on_step=True)
        self.log("tau_high", tau_high.detach(), on_step=True)
        self.log("tau_low",  tau_low.detach(), on_step=True)
        self.log("ratio_sr_sy", ratio.detach(), on_step=True)
