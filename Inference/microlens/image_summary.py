...
def gpu_computation(batch, rank):
    # 根据rank确定设备，只选设备字符串，不调用model.to()避免权重在多GPU分散
    ...
    # processor 返回的是字典，将所有tensor放到同一device
    ...
    # 这里假设 model 已经初始化好且在默认GPU上（或者调用前放到device）
    # 只用 model.generate 调用，确保 model 在device上
    with torch.no_grad():
        with autocast():
